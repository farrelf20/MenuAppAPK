...
import com.example.menuapp.Callback.IRecyclerItemClickListener
import com.example.menuapp.EventBus.CategoryClick
...
class MyCategoriesAdapter (internal var context: Context,
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        holder.setListener(object:IRecyclerItemClickListener{
            override fun onItemClick(view: View, pos: Int) {
                com.example.menuapp.Common.Common.categorySelected = categoriesList.get(pos)
    ...
    override fun getItemViewType(position: Int): Int {
        return if(categoriesList.size == 1)
            com.example.menuapp.Common.Common.DEFAULT_COLUMN_COUNT
        else{
            if(categoriesList.size % 2 == 0)
                com.example.menuapp.Common.Common.DEFAULT_COLUMN_COUNT
            else
                if(position > 1 && position == categoriesList.size-1) com.example.menuapp.Common.Common.FULL_WIDTH_COLUMN else com.example.menuapp.Common.Common.DEFAULT_COLUMN_COUNT
        }
    }
